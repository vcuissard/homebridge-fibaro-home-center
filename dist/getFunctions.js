...
class GetFunctions {
    ...
    getCurrentTiltAngle(characteristic, service, IDs, properties) {
        ...
        const angle = setFunctions_1.SetFunctions.scale(value2, 0, 100, characteristic.props.minValue, characteristic.props.maxValue);
    ...
    RGBtoHSV(r, g, b, w) {
        ...
        const max = Math.max(r, g, b), min = Math.min(r, g, b), d = max - min, s = (max === 0 ? 0 : d / max), v = Math.max(max, w) / 255;
        let h;
        switch (max) {
            case min:
                h = 0;
